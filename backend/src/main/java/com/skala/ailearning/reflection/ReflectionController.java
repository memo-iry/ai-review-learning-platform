...
public class ReflectionController {
        ...
        @Operation(summary = "회고 저장", description = """
                        수업 후 작성한 회고를 저장한다.
        ...
        @ApiResponses({
                        ...
                        @ApiResponse(responseCode = "404", description = "사용자 또는 강의 없음", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
        @PostMapping
        ...
        @Operation(summary = "회고 전체 목록 조회", description = """
                        특정 사용자가 작성한 회고를 최신순으로 모두 조회한다.
                        """)
        @ApiResponses({
        ...
        @GetMapping
        public List<ReflectionResponse> list(@RequestParam Long userId) {
                accessGuard.requireSelfOrAdmin(userId);
                return reflectionService.findAllByUser(userId);
        }

        @Operation(summary = "이해도 분석 및 복습자료 생성", description = """
        ...
        @ApiResponses({
                        ...
                        @ApiResponse(responseCode = "404", description = "회고 없음", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
        })
